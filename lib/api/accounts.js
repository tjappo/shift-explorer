...
module.exports = function (app) {
	...
	function Account() {
		...
		this.getAccount = (address, cb) =>
			request.get({
				url: `http://${config.shift.host}:${config.shift.port}/api/accounts?address=${address}`,
				json: true,
			}, (err, response, body) => {
		...
		this.getAccountByPublicKey = (publicKey, cb) =>
			request.get({
				url: `http://${config.shift.host}:${config.shift.port}/api/accounts?publicKey=${publicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getDelegate = (account, cb) => {
			...
			return request.get({
				url: `http://${config.shift.host}:${config.shift.port}/api/delegates/get?publicKey=${account.publicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getVotes = (account, cb) => {
			...
			return request.get({
				url: `http://${config.shift.host}:${config.shift.port}/api/accounts/delegates/?address=${account.address}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getVoters = (account, cb) => {
			...
			return request.get({
				url: `http://${config.shift.host}:${config.shift.port}/api/delegates/voters?publicKey=${account.publicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getIncomingTxsCnt = (account, cb) => {
			...
			return request.get({
				url: `http://${config.shift.host}:${config.shift.port}/api/transactions?recipientId=${account.address}&limit=1`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getOutgoingTxsCnt = (account, cb) => {
			...
			return request.get({
				url: `http://${config.shift.host}:${config.shift.port}/api/transactions?senderId=${account.address}&limit=1`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getForged = (account, cb) => {
			...
			return request.get({
				url: `http://${config.shift.host}:${config.shift.port}/api/delegates/forging/getForgedByAccount?generatorPublicKey=${account.publicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getTopAccounts = (query, error, success) => {
		...
		return request.get({
			url: `http://${config.shift.host}:${config.shift.port}/api/accounts/top?&offset=${param(query.offset, 0)}&limit=${param(query.limit, 100)}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {

...
module.exports = function (app) {
	...
	this.getBlocks = function (query, error, success) {
		...
		async.doUntil(
			(next) => {
				request.get({
					url: `http://${config.shift.host}:${config.shift.port}${blocks.url(offset, limit)}`,
					json: true,
				}, (err, resp, body) => {
	...
	this.getLastBlock = function (query, error, success) {
		...
		request.get({
			url: `http://${config.shift.host}:${config.shift.port}${blocks.url(0, 1)}`,
			json: true,
		}, (err, resp, body) => {
			if (err || resp.statusCode !== 200) {
	...
	this.getPeers = function (query, error, success) {
		...
		async.doUntil(
			(next) => {
				request.get({
					url: `http://${config.shift.host}:${config.shift.port}${peers.url(offset, limit)}`,
					json: true,
				}, (err, resp, body) => {
					if (err || resp.statusCode !== 200) {
						return next(err || 'Response was unsuccessful');
